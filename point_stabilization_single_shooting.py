...
def create_solver(N, T, Q, R):
    ...
    P = cas.SX.sym('P', n_states + n_states) # type: ignore
    ...
    for i in range(N):
        ...
        obj += (state - P[n_states:]).T @ Q @ (state - P[n_states:]) \
            + control.T @ R @ control
...
if __name__ == "__main__":
    ...
    R = np.diag([1.0, 0.4]) # Input weighting

    solver, ff = create_solver(N, T, Q, R)
